...
function cards() {
  ...
  cards2.forEach((card) => {
    ...
    const checkboxItems = card.querySelectorAll(".cards__item--checkbox");
    checkboxItems.forEach((item) => {
      const checkbox = item.querySelector(".cards__item-checkbox input[type=checkbox]");
      if (checkbox)
        checkbox.checked = false;
      item.classList.remove("cards__item--checkbox-active");
      item.addEventListener("click", () => {
        checkbox.checked = !checkbox.checked;
        checkbox.checked ? item.classList.add("cards__item--checkbox-active") : item.classList.remove("cards__item--checkbox-active");
      });
    });
  });
}
...
function b24() {
  const bitrixСallBtns = document.querySelectorAll(".call-b24");
...
function bookmarks() {
  document.querySelectorAll(".bookmarks").forEach((bookmark) => {
    bookmark.querySelectorAll(".bookmarks__item").forEach((item) => {
      const bookmarkPreview = item.querySelector(".bookmarks__item-preview");
      if (bookmarkPreview) {
        bookmarkPreview.addEventListener("click", () => {
          const isActive = item.classList.toggle("bookmarks__item--active");
          if (isActive) {
        ...
        const closePopup = item.querySelector(".bookmarks__item-popup-close");
        if (closePopup)
          closePopup.addEventListener("click", () => {
            item.classList.remove("bookmarks__item--active");
          });
      }
    });
  });
...
const handleDocumentClick = (e2) => {
  if (!e2.target.closest(".bookmarks__item-popup") && !e2.target.closest(".bookmarks__item-preview")) {
    document.querySelectorAll(".bookmarks__item--active").forEach((activeItem) => {
      activeItem.classList.remove("bookmarks__item--active");
    });
  }
};
